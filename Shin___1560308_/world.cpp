    ...
    if (m_toggle) {
        for (char i = 0; i < MAX_ROWS; i++) {
            for (char j = 0; j < MAX_COLS; j++) {
                ...
                if (m_otherWorld[i][j] != m_world[i][j]){
                    if (m_otherWorld[i][j]=='X' && m_world[i][j]=='.'){
                        stats().record(i, j, 1);
                    }
                }
            }
        ...
        for (char i = 0; i < MAX_ROWS; i++) {
            for (char j = 0; j < MAX_COLS; j++) {
                ...
                if (m_world[i][j] != m_otherWorld[i][j]){
                if (m_world[i][j]=='X' && m_otherWorld[i][j]=='.'){
                    stats().record(i, j, 1);
                    }
                }
            }
...
bool World::addLife(Life *life) {
    ...
    for (int i = life->getRow(); i - life->getRow() < life->getHeight(); i++) {
        for (int j = life->getCol(); j - life->getCol() < life->getWidth(); j++) {
            if (i < MAX_ROWS && j < MAX_COLS) {
                m_world[i][j] = life->getFromFigure(i - life->getRow(), j - life->getCol());
            }
        }
    }
    return true;
